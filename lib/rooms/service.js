...
import { hashAccessCode, validateAccessCodeFormat, generateAccessCode } from '../security/access-code.js';
...
export async function createRoom({
  ...
  let room;
  let slug;
  let resolvedAccessCode;
  for (let attempt = 0; attempt < 5; attempt += 1) {
    slug = generateRoomSlug();
    resolvedAccessCode = (typeof accessCode === 'string' && accessCode.trim())
      ? accessCode.trim()
      : slug;

    const codeFormat = validateAccessCodeFormat(resolvedAccessCode, {
    ...
    const codeHash = await hashAccessCode(resolvedAccessCode);
  ...
  const invite = await repo.insertInvite({
    ...
    expires_at: expiresAtIso,
    max_uses: policy.inviteDefaultMaxUses ?? null,
    used_count: 0,
    revoked_at: null,
  });
