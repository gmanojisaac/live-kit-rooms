 ...
 */
export function evaluateInvite(invite, { now = Date.now, roomExpiresAt = null, allowLegacyExhaustion = false } = {}) {
  const credentials = evaluateInviteCredentials(invite, { now, roomExpiresAt });
  if (!credentials.ok) return credentials;

  if (invite.max_uses != null && invite.used_count >= invite.max_uses && !allowLegacyExhaustion) {
...
export async function validateRoomAccess({
  ...
  let invite = null;
  if (typeof rawInviteToken === 'string') {
    const trimmed = rawInviteToken.trim();
    if (!trimmed) {
      return { ok: false, reason: AccessRejection.INVITE_INVALID, httpStatus: 403 };
    }
    let tokenHash;
    try {
      tokenHash = hashInviteToken(trimmed);
    } catch {
      return { ok: false, reason: AccessRejection.INVITE_INVALID, httpStatus: 403 };
    }
    ...
    const credentialsCheck = evaluateInviteCredentials(invite, {
      now,
      roomExpiresAt: room.expires_at,
    });
    if (!credentialsCheck.ok) {
      return { ok: false, reason: credentialsCheck.reason, httpStatus: 403 };
    }
  } else {
  ...
  let grantCount = 0;
  if (invite?.id && typeof repository.countParticipantRejoinGrantsForInvite === 'function') {
    try {
      grantCount = await repository.countParticipantRejoinGrantsForInvite(invite.id);
    } catch {
      grantCount = 0;
    }
  }

  const isLegacyInvite = invite?.max_uses != null
    && invite.used_count >= invite.max_uses
    && grantCount < invite.max_uses;

  const inviteCheck = evaluateInvite(invite, {
    now,
    roomExpiresAt: room.expires_at,
    allowLegacyExhaustion: isLegacyInvite,
  });
  if (!inviteCheck.ok) {
    if (
