...
import { generateRejoinToken } from '../security/rejoin-token.js';
...
const AUTH_FAILURE_REASONS = new Set([
  ...
  AccessRejection.REJOIN_INVALID,
  AccessRejection.REJOIN_REVOKED,
]);
...
function mapAccessRejection(reason, httpStatus) {
  switch (reason) {
    ...
    case AccessRejection.REJOIN_REVOKED:
      return new RoomJoinError('Your rejoin permission was revoked.', {
        httpStatus: 403,
        code: reason,
      });
    case AccessRejection.REJOIN_INVALID:
      return new RoomJoinError('Invalid rejoin credential.', {
        httpStatus: 403,
        code: reason,
      });
    default:
 ...
 *   participant: { identity: string, displayName: string, rejoinToken: string },
...
export async function joinRoomAsParticipant({
  ...
  if (!isPlausibleInviteToken(inviteToken)) {
  ...
  if (typeof accessCode !== 'string' || accessCode.length < MIN_ACCESS_CODE_LENGTH) {
    throw new RoomJoinError('Invalid invitation or access code.', {
      httpStatus: 403,
  ...
  const { room, invite, rejoinGrant, isRejoin } = access;
  ...
  let rawRejoinToken = typeof rejoinToken === 'string' && rejoinToken ? rejoinToken : null;

  if (isRejoin && rejoinGrant) {
    if (typeof repo.touchParticipantRejoinGrant === 'function') {
      await repo.touchParticipantRejoinGrant(rejoinGrant.id, {
        participantIdentity: identity,
        lastUsedAt: new Date(now()).toISOString(),
      });
    }
  } else {
    const minted = generateRejoinToken();
    rawRejoinToken = minted.rawToken;
    let grantRow = null;
    try {
      grantRow = await repo.insertParticipantRejoinGrant({
        room_id: room.id,
        invite_id: invite.id,
      ...
      throw new RoomJoinError('Unable to prepare rejoin credentials.', {
        httpStatus: 500,
        code: 'REJOIN_GRANT_FAILED',
      });
    }

    // Consume only after auth + capacity + token mint + rejoin grant succeed.
    const consumed = await repo.tryConsumeInvite(invite.id);
    if (!consumed) {
      try {
        await repo.deleteParticipantRejoinGrant(grantRow.id);
      } catch {
