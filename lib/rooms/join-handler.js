...
import { getOwnerSessionToken, resolveClientIp } from '../http/request.js';
...
export async function handleJoinPost(request, { params }, deps = {}) {
  ...
  const rejoinToken = body?.rejoinToken ?? null;
  ...
  try {
    const result = await (deps.joinRoomAsParticipant || joinRoomAsParticipant)({
      ...
      ownerSessionToken: getOwnerSessionToken(request),
      ownerJoinToken: body?.ownerJoinToken,
      policy,
    ...
    return {
      ...
      body: {
        ...
        moderatorToken: result.moderatorToken || null,
      },
      headers,
    };
  } catch (error) {
