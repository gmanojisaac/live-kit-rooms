 ...
 */
export function saveOwnerJoinHandoff({ slug, displayName, accessCode }, storage, now = Date.now) {
  ...
  try {
    store.setItem(OWNER_JOIN_HANDOFF_KEY, JSON.stringify({
      slug,
      displayName: name,
      accessCode: code,
      savedAt: now(),
 ...
 * @returns {{ displayName: string, accessCode: string } | null}
 */
export function readOwnerJoinHandoff(slug, storage, now = Date.now) {
  ...
  return { displayName: name, accessCode: code };
}
