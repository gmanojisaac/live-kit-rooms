...
export function createRoomRepository({ client, env } = {}) {
  ...
  return {
     ...
     * Record an invite use after admission succeeds. This is informational only:
     * room capacity is enforced from LiveKit's connected participant count.
    ...
    async tryConsumeInvite(inviteId) {
      ...
      const room = await this.getRoomById(invite.room_id);

      if (!room || room.status !== 'active') return null;

      if (room.expires_at && Date.parse(room.expires_at) <= Date.now()) {

        if (room.status === 'active') await this.markRoomExpired(room.id);
    ...
    async countParticipantRejoinGrantsForInvite(inviteId) {
  ...
  return {
    ...
    async tryConsumeInvite(inviteId) {
      ...
      for (const grant of rejoinGrants.values()) {
        if (grant.invite_id === inviteId) grantCount += 1;
      }
      if (invite.max_uses != null && invite.used_count >= invite.max_uses && grantCount >= invite.max_uses) {
        return null;
      }
    ...
    async countParticipantRejoinGrantsForInvite(inviteId) {
      let count = 0;
      for (const grant of rejoinGrants.values()) {
        if (grant.invite_id === inviteId) count += 1;
      }
      return count;
    },
