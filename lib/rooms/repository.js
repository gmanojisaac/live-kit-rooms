...
export function createRoomRepository({ client, env } = {}) {
  ...
  // Never pass process.env into the client factory under Next.js — the env
  // object can be a filtered map that drops server secrets. Prefer no-arg
  // createServiceRoleSupabaseClient() so secrets are read via static access.
  // Pass a custom `env` object only from tests (never process.env itself).
